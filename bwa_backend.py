...
llm = ChatOpenAI(model=OPENAI_MODEL, temperature=0.7, max_retries=3)
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            ...
            try:
                ...
                logger.warning("tavily_search: timeout after %.1fs", timeout_seconds)
                return []
        out: List[dict] = []
        for r in results or []:
            out.append(
...
def _limit_query_payload(results: List[dict], max_tokens: int = 8000) -> List[dict]:
    if not results:
        return []
...
def research_node(state: State) -> dict:
    ...
    raw: List[dict] = []
    for q in queries:
        logger.info("research_node: query start q=%s", q)
        t0 = time.perf_counter()
        results = _tavily_search(q, max_results=6)
        results = _limit_query_payload(results, max_tokens=8000)
        raw.extend(results)
        logger.info(
            "research_node: query done q=%s results=%d duration=%.2fs",
            q,
            len(results),
...
def _openai_generate_image_bytes(prompt: str, size: str, quality: str) -> bytes:
    """
